extends Node2D
class_name BaseTransitionScreen
...
func _setup_transition():
	...
	shader_meterial.set("shader_parameter/color", fade_color)
	shader_meterial.set("shader_parameter/diamond_size", diamond_size)
	shader_meterial.set("shader_parameter/blur_intensity", blur_intensity)
	shader_meterial.set("shader_parameter/min_pixel", min_pixel)
	shader_meterial.set("shader_parameter/max_pixel", max_pixel)
	shader_meterial.set("shader_parameter/transition_type", trainsition_type)
	...
	get_viewport().size_changed.connect(_update_screen_size_changed)
...
func _set_fade_value(value: float):
	fade_value = value
	self.shader_meterial.set("shader_parameter/process_value", value)
